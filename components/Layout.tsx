...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* PERSISTENT SIDEBAR */}
      <aside className="hidden lg:flex fixed top-0 left-0 bottom-0 w-[280px] bg-white border-r border-gray-100 flex-col z-50">
        <div className="p-8">
          <Link to="/" className="flex items-center gap-3 group">
            <div className="w-10 h-10 bg-indigo-600 rounded-2xl flex items-center justify-center text-white shadow-xl shadow-indigo-200 group-hover:rotate-12 transition-transform duration-500">
              <span className="text-xl font-black italic">N</span>
            </div>
            <div>
              <h1 className="text-xl font-black tracking-tighter text-gray-900 leading-none">NovaMart<span className="text-indigo-600">AI</span></h1>
              <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mt-0.5">Neural Commerce</p>
            </div>
          </Link>
        </div>

        <nav className="flex-grow px-4 space-y-8 overflow-y-auto hide-scrollbar py-4">
          <div className="space-y-1.5">
            ...
            <NavLink to="/wishlist" icon="💖" label={t.wishlist} badge={wishlist.length} colorClass="pink" />
          ...
          {user?.role === 'admin' && (
            <div className="space-y-1.5">
              ...
              <NavLink to="/admin" icon="🛡️" label={t.admin} colorClass="gray" />
            </div>
          )}
        </nav>
        ...
        <div className="p-4 mt-auto border-t border-gray-50">
          {user ? (
            ...
            <Link to="/login" className="flex items-center justify-center gap-2 w-full bg-gray-900 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-gray-200 hover:bg-gray-800 active:scale-95 transition-all">
              {t.signin}
            </Link>
          )}
          <div className="mt-4 flex bg-gray-100 rounded-xl p-1">
            <button onClick={() => setLocale('en')} className={`flex-1 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest ${locale === 'en' ? 'bg-white shadow-sm text-indigo-600' : 'text-gray-400'}`}>EN</button>
            <button onClick={() => setLocale('hi')} className={`flex-1 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest ${locale === 'hi' ? 'bg-white shadow-sm text-indigo-600' : 'text-gray-400'}`}>हि</button>
          </div>
        </div>
      </aside>

      {/* TOP HEADER */}
      <header className="lg:ml-[280px] sticky top-0 z-40 bg-white/80 backdrop-blur-xl border-b border-gray-100 h-16 sm:h-20 flex items-center px-4 lg:px-10 justify-between">
        <div className="flex items-center gap-4">
          <button 
            onClick={() => setIsMenuOpen(true)}
            className="lg:hidden p-2.5 bg-gray-50 rounded-xl text-gray-600 active:scale-90 transition-all"
          >
          ...
          <div className="lg:hidden">
            <h1 className="text-lg font-black tracking-tighter text-gray-900 leading-none">NovaMart<span className="text-indigo-600">AI</span></h1>
          </div>
        </div>
      ...
      <div className={`lg:hidden fixed inset-0 z-[100] transition-all duration-500 ${isMenuOpen ? 'visible opacity-100' : 'invisible opacity-0'}`}>
        <div className="absolute inset-0 bg-gray-900/60 backdrop-blur-md" onClick={() => setIsMenuOpen(false)}></div>
        <div className={`absolute top-0 left-0 bottom-0 w-[85%] max-w-[320px] bg-white shadow-2xl flex flex-col p-6 transition-transform duration-500 transform ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <div className="flex justify-between items-center mb-10">
            <h2 className="text-xl font-black tracking-tighter text-gray-900">NovaMart<span className="text-indigo-600">AI</span></h2>
            <button onClick={() => setIsMenuOpen(false)} className="p-2 text-gray-400 hover:text-red-500"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg></button>
          </div>
      ...
      <main className="lg:ml-[280px] p-4 lg:p-10 max-w-7xl mx-auto">
        {children}
      </main>

      <footer className="lg:ml-[280px] bg-white border-t py-12 px-4 lg:px-10">
